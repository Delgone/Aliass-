...
const customSelect = document.getElementById('word-pack-select');
const selectValueEl = customSelect.querySelector('.select-value');
const optionEls = customSelect.querySelectorAll('.select-option');

// текущий ключ набора
let currentPackKey = selectValueEl.dataset.pack || 'classic';
...
// старт раунда
function startRound() {
  // выбираем нужный набор по значению селекта
  const packKey = currentPackKey;
  words = wordPacks[packKey] || wordPacks.classic;
...
circle.addEventListener('touchend', () => {
  ...
  // вниз или вверх — следующее слово
  currentIndex++;
  showWord();
...
optionEls.forEach(option => {
  option.addEventListener('click', (e) => {
    ...
    currentPackKey = pack;
    selectValueEl.dataset.pack = pack;
    selectValueEl.textContent = text;
